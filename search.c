...
static const ub8 Timelimit = 1500;
      ...
      if (t > last || dcnt == 0) {  // new entry
        ...
        while (--dndx) {
          ...
          if (devp[timefld] == t) {
            ...
            warn(0,"hop %u time \ad%u not found for \ad%u",hop,t,last);
            break;
          }
        }
      } // new entry or not
...
//      info(Notty,"mid %u depmid %u midarr %u",mid,n1,n2);
      ...
      if (havetime && stop > 2) {
        src->querytlim = min(src->querytlim,src->queryt0 + 1000 * 300);
...
//      if (dist == 0) warning(0,"dist 0 for len %u",nleg);
  ...
  if (havetime) {
    ...
    if (stop == 0) src->querytlim = min(src->querytlim,src->queryt0 + 1000 * 200);
    return 1;
  }
...
static ub4 dosrc(struct gnetwork *gnet,ub4 nstoplo,ub4 nstophi,search *src,char *ref)
{
  ...
  if (partcnt == 0) { error(0,"search called without partitions, ref %s",ref); return 0; }
    ...
    for (stop = nstoplo; stop <= nstophi; stop++) {
      ...
      if (gettime_usec() > src->querytlim) return allconn;
    }
    return allconn;
  }
...
int plantrip(search *src,char *ref,ub4 xdep,ub4 xarr,ub4 nstoplo,ub4 nstophi)
{
  ...
  src->querytlim = t0 + 1000UL * Timelimit;

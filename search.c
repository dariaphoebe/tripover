...
static ub4 nxtevs(search *src,lnet *net,ub4 leg,ub4 bstop,ub4 hop,ub4 midur,ub4 costlim)
{
  ...
  while (adndx < adcnt && prvgndx < gencnt && dcnt < dmax) {
    ...
    while (gndx < gencnt && dcnt < dmax) {
      ...
      cost = acost + t - atarr;
      if (cost >= costlim) { src->stat_nxtlim++; break; }

      if (t > atarr + ttmax) {
        if ( (ddcnt > 8 && ddcnt != dcnt) || (ddcnt > 64) ) {
          src->stat_nxt3++;
          break;
        }
        ddcnt++;
...
//      info(Notty,"mid %u depmid %u midarr %u",mid,n1,n2);
      ...
      if (havetime && stop > 2) {
        src->querytlim = min(src->querytlim,src->queryt0 + 1000 * 600);
...
//              src->lodt = lodt;
            ...
            if (nleg > 3) {
              src->querytlim = min(src->querytlim,src->queryt0 + 1000 * 1000);
...
//      if (dist == 0) warning(0,"dist 0 for len %u",nleg);
  ...
  if (havetime) {
    ...
    if (stop == 0) src->querytlim = min(src->querytlim,src->queryt0 + 1000 * 500);
    return 1;
  }
